"""Benchmark a CSV manifest.

The file needs an ``image_path`` column and a label column::

    image_path,class_name
    images/cat/cat_001.jpeg,cat

Relative paths resolve from the manifest's own directory, so a manifest and
its images move together.

Note that ``target_labels`` means something different here than for folder
input: it is the *name of the label column*, a single string.
...
def main():
    ...
    print(f"csv input: {manifest}")
    results = benchmark_image_classification(
        dataset=str(manifest),
        dataset_type="csv",
        target_labels="class_name",   # a column name, not a list
        color_mode="rgb",
    )

"""The RGB demonstration.

Three-channel input: every image becomes ``(64, 64, 3)``, and the classical
models see 64 x 64 x 3 = 12,288 features each.

Pair this with ``grayscale_example.py``, which runs the same images through
the same pipeline with one channel. The difference between the two is what
color was worth on this dataset.
